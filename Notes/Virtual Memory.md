...
- A specific application of the [[Memory Hierarchy]]: it uses main memory as a "cache" for secondary storage, with a virtual memory block called a page and a virtual memory miss called a [[Page Fault|page fault]] (an event triggered when an accessed page is not present in main memory).[^2]
- Pages are loaded and allocated only as a process needs them under [[Demand Paging]], rather than all being made available up front.
