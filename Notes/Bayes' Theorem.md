...
The original [[Probability|probability]] $P(\theta)$ is called the prior, while the new/updated [[Conditional Probability|conditional probability]] $P(\theta | Y)$ is called the posterior.[^1] $P(Y | \theta)$ is the likelihood of the data given the parameters: the [[Likelihood Function|likelihood function]] $L(\theta | Y)$ is, by definition, the same quantity $P(Y | \theta)$ viewed as a function of $\theta$ for fixed observed $Y$. In other words, given the observed data $Y$, the probability of parameters $\theta$ gets updated.
