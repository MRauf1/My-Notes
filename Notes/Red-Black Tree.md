...
The root can always be coloured black without violating either property, so it is taken to always be black; treating every [[External Node|external node]] (nil) as black as well means every real node has exactly two children, each with a well-defined colour.[^2] Red-black trees are one of the most widely used data structures in practice — e.g. as the primary SSet in the Java Collections Framework, several C++ Standard Template Library implementations, and the Linux kernel — because, compared to a [[Skiplist]], [[Treap]], or [[Scapegoat Tree]], they guarantee $O(\log n)$ *worst-case* time for `add(x)`/`remove(x)` (rather than only expected or amortized time) while also performing only $O(1)$ *amortized* rotations per update.[^3]
...
> A RedBlackTree implements the SSet interface and supports `add(x)`, `remove(x)`, and `find(x)` in $O(\log n)$ worst-case time per operation.
