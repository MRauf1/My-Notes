...
- Simultaneous multithreading (SMT) — lowers the cost of multithreading by reusing the resources already present in a [[Multiple Issue|multiple-issue]], dynamically scheduled microarchitecture (see [[Dynamic Pipeline Scheduling]]), issuing instructions from several threads within the same clock cycle rather than switching wholesale between them.
