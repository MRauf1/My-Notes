...
2) Valid [[Objective Function]] present, but too expensive, so the designer relies on approximations and on the model's capability to extrapolate on limited samples, which can be dangerous.
